...
import type {
  FormularyPrescriptionClinicalContextPort,
...
import {
  ...
  PrescriptionEncounterNotActiveError,
  PrescriptionEncounterUnsignedProviderError,
...
export class FormularyPrescriptionContextService
implements FormularyPrescriptionClinicalContextPort {
  public constructor(
  ...
  public async resolveActiveEncounter(
    ...
    if (
      ...
      throw new PrescriptionEncounterNotActiveError();
    }
    ...
    if (clinic !== null) {
      if (clinic.status !== 'ACTIVE') {
        mismatch(
          'The encounter clinic is inactive',
        );
      }
    ...
    if (servicePoint !== null) {
      if (servicePoint.status !== 'ACTIVE') {
        mismatch(
          'The encounter service point is inactive',
        );
      }
    ...
    if (
      ...
      if (
        registration.patientId !==
          encounter.patientId ||
        registration.requestedPatientId !==
          encounter.requestedPatientId ||
    ...
    if (
      ...
      if (
        ...
        mismatch(
          'The OPD visit is not eligible for prescribing',
        );
      }
    ...
    if (
      ...
      if (
        queueToken.registrationId !==
          encounter.registrationId ||
        queueToken.opdVisitId !==
          encounter.opdVisitId ||
        queueToken.patientId !==
          encounter.patientId
      ) {
        mismatch(
  ...
  public assertPrescriberAssigned(
    context: PrescriptionClinicalContext,
    prescriberProviderId: string,
  ): void {
    if (
      !context.assignedProviderIds.includes(
        prescriberProviderId,
      )
    ) {
      throw new PrescriptionEncounterUnsignedProviderError();
    }
  }
}