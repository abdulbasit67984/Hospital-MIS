...
export interface RadiologyEncounterContextRecord {
  id: string;
  facilityId: string;
  patientId: string;
...
export interface RadiologyActorIdentityRecord {
  userId: string;
...
export class RadiologyContextRepository {
  public async findEncounter(
    facilityId: string,
    encounterId: string,
  ): Promise<RadiologyEncounterContextRecord | null> {
    const record = await EncounterModel.findOne({
    ...
    const assignedProviderIds = new Set(
      record.assignedProviderIds.map((providerId) =>
        providerId.toHexString(),
      ),
    );

    assignedProviderIds.add(record.primaryProviderId.toHexString());
    assignedProviderIds.add(record.currentOwnerId.toHexString());
  ...
  public async findActorIdentity(
    ...
    const record = await UserModel.findById(
      ...
      .lean<{
        ...
        status: RadiologyActorIdentityRecord['status'];
      }>()
      .exec();
